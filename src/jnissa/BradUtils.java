...
public class BradUtils extends Object
{
	...
	// does mod 12 arithmetic on oct.pc numbers
	static double pchadd(double val1, double val2)
	{
		int intpart;
		double fractpart;

		intpart = (int)val1 + (int)val2;
		fractpart = (val1 - (double)((int)val1)) + (val2 - (double)((int)val2));

		while (fractpart >= 0.12)
		{
			fractpart -= 0.12;
			intpart++;
		}
		...
		return((double)intpart + fractpart);
	}
}
